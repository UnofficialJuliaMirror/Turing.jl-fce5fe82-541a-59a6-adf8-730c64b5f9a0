...
@model infiniteMixture(y) = begin
  ...
  k = 0
  T = 1
  T_surplus = Array{Real}(N)
  T_surplus[1] = T

  if sampler != nothing && sampler.alg.gid == 1
    max_index = maximum([parse(split(split(vn.indexing,"[")[end], "]")[1]) for vn in vi.vns if vn.sym == :V])
    for j in 1:max_index
      V[j] ~ V_SBS_DP(10)
      x[j] ~ H
      T_surplus[j+1] = T_surplus[j] - V[j]*T_surplus[j]
    end
    k = max_index
  end
...
# vi = model()
# sampler = HMC(10, 0.03, 5,:V,:x)
sampler = Gibbs(5, CSMC(1, 1, :z), HMC(1, 0.03, 5,:V,:x))
